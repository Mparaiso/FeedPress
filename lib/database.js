    ...
    container.share('db', function (container) {
            ...
            FeedSchema.statics.unsubscribe = function (id, cb) {
                return Feed.where().findOneAndRemove({_id:id}, function (err, feed) {
                    err ? cb(err) : Article.where("meta.xmlurl").equals(feed.xmlurl).remove(function (err) {
                        return cb(err, feed);
                    });
                });
            };
            ...
            CategorySchema.post("remove",function(category){
                // clean up
                Feed.find({_category:category._id},function(err,feeds){
                    console.log("category post remove,found : ",feeds);
                    for(var i in feeds){
                        feeds[i]._category=null;
                        feeds[i].save();
                    }
                });
            });
