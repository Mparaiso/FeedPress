    ...
    container.share('db', function (container) {
            ...
            FeedSchema.statics.subscribe = function (url, callback) {
                return container['feedparser'].parse(url, function (err, meta, articles) {
                    ...
                    container.logger.log("meta",meta.xmlurl,url);
                    return Feed.update({xmlurl:meta.xmlurl}, meta, {upsert:true},
                        function (err, affectedDocs, mongoResponse) {
                            container.logger.log("feed updated", arguments);
                            if (err)return callback(err);
                            return Article.find({'meta.xmlurl':meta.xmlurl}, 'link', function (err, _articles) {
                                container.logger.log('from subscribe:', _articles.length,' articles found');
