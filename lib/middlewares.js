...
module.exports = {
        ...
        db.model("Category").find().lean().populate({path:'_feeds', select:'_id title xmlurl favicon _read _articles'}).exec(
            function (err, categories) {
                ...
                db.model("Feed").populate(categories, {
					...
					console.dir(categories[0]._feeds);
     ...
     */
    getLastSync:function(req,res,next){
        var db = req.app.DI.db;
        db.model("Sync").findOne({},"date",{sort:{date:-1}},function(err,sync){
            if(err)req.app.DI.logger.error(err);
            if(sync){
                res.locals.last_sync_date = sync.date;
            }
            next();
        });
    }
